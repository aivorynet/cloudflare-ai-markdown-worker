...
var require_CSSStyleSheet = __commonJS({
  "node_modules/cssom/lib/CSSStyleSheet.js"(exports) {
    ...
    CSSOM.CSSStyleSheet.prototype.toString = function() {
      var result = "";
      var rules2 = this.cssRules;
      for (var i = 0; i < rules2.length; i++) {
        result += rules2[i].cssText + "\n";
      }
      return result;
    };
...
var require_clone = __commonJS({
  "node_modules/cssom/lib/clone.js"(exports) {
    ...
    CSSOM.clone = function clone(stylesheet) {
      ...
      var rules2 = stylesheet.cssRules;
      if (!rules2) {
        return cloned;
      }
      for (var i = 0, rulesLength = rules2.length; i < rulesLength; i++) {
        var rule = rules2[i];
...
function filterValue(rule, node, options) {
  ...
  if (typeof filter2 === "string") {
    ...
    if (filter2.call(rule, node, options))
      return true;
  } else {
  ...
  while (node !== element) {
    if (node.nodeType === 3 || node.nodeType === 4) {
      ...
      if (isBlock2(node) || node.nodeName === "BR") {
        ...
        prevText = null;
        keepLeadingWs = false;
      } else if (isVoid3(node) || isPre(node)) {
        prevText = null;
        keepLeadingWs = true;
      } else if (prevText) {
        keepLeadingWs = false;
...
var knownAdjacent = (prev, next2) => {
  prev[NEXT] = next2;
  next2[PREV] = prev;
};
var knownBoundaries = (prev, current, next2) => {
  knownAdjacent(prev, current);
  knownAdjacent(getEnd(current), next2);
};
var knownSegment = (prev, start, end, next2) => {
  knownAdjacent(prev, start);
  knownAdjacent(getEnd(end), next2);
};
var knownSiblings = (prev, current, next2) => {
  knownAdjacent(prev, current);
...
var remove2 = (prev, current, next2) => {
  const { parentNode, nodeType } = current;
  if (prev || next2) {
    setAdjacent(prev, next2);
    current[PREV] = null;
...
function getChildFunc(next2, adapter2) {
  return (elem) => {
    ...
    return parent != null && adapter2.isTag(parent) && next2(elem);
  };
}
var filters = {
  contains(next2, text, { adapter: adapter2 }) {
    return function contains(elem) {
      return next2(elem) && adapter2.getText(elem).includes(text);
    };
  },
  icontains(next2, text, { adapter: adapter2 }) {
...
function compilePseudoSelector(next2, selector, options, context, compileToken2) {
  ...
  if (Array.isArray(data)) {
    ...
    return subselects[name](next2, data, options, context, compileToken2);
  }
  ...
  if (typeof userPseudo === "function") {
    ...
    return (elem) => userPseudo(elem, data) && next2(elem);
  }
  if (name in filters) {
    return filters[name](next2, data, options, context);
  }
  if (name in pseudos) {
    const pseudo = pseudos[name];
    verifyPseudoArgs(pseudo, name, data, 2);
    return (elem) => pseudo(elem, options, data) && next2(elem);
  }
  ...
  switch (selector.type) {
    ...
    case SelectorType.Child: {
      return function child(elem) {
        const parent = adapter2.getParent(elem);
        return parent != null && adapter2.isTag(parent) && next2(parent);
      };
    }
    case SelectorType.Sibling: {
...
function compileRules(rules2, options, context) {
  var _a3;
  return rules2.reduce((previous, rule) => previous === import_boolbase5.default.falseFunc ? import_boolbase5.default.falseFunc : compileGeneralSelector(previous, rule, options, context, compileToken), (_a3 = options.rootFunc) !== null && _a3 !== void 0 ? _a3 : import_boolbase5.default.trueFunc);
}
...
var matches = (element, selectors) => is3(
  element,
  selectors,
  {
    strict: true,
...
var ParentNode = class extends Node3 {
  ...
  replaceChildren(...nodes) {
    ...
    while (next2 !== end && next2.nodeType === ATTRIBUTE_NODE)
      next2 = next2[NEXT];
    while (next2 !== end) {
      const after2 = getEnd(next2)[NEXT];
      next2.remove();
      next2 = after2;
    }
    if (nodes.length)
...
var isVoid2 = ({ localName, ownerDocument }) => {
...
var Element2 = class extends ParentNode {
  ...
  cloneNode(deep = false) {
    ...
    let { [NEXT]: next2, [END]: prev } = this;
    while (next2 !== prev && (deep || next2.nodeType === ATTRIBUTE_NODE)) {
      switch (next2.nodeType) {
        case NODE_END:
        ...
        case ATTRIBUTE_NODE: {
          const attr = next2.cloneNode(deep);
...
var deleteContents = ({ [START]: start, [END]: end }, fragment = null) => {
  ...
  do {
    ...
    const next2 = after2 === end ? after2 : after2[NEXT];
    if (fragment)
...
function convertHtmlToMarkdown(html, pathname) {
  ...
  const titleElement = document2.querySelector("title");
  const title = titleElement ? titleElement.textContent.trim() : "Page Content";
  let contentElement = null;
  for (const selector of CONFIG.contentSelectors) {
    try {
      contentElement = document2.querySelector(selector);
      if (contentElement) {
        break;
      }
    } catch (e) {
  ...
  if (!contentElement) {
    contentElement = document2.body || document2.documentElement;
  }
